...
public class PolygonIntersect {
	 ...
	 */
	public static double intersectionArea(Position[] a, Position[] b) {
	...
	private static void range(Position[] points, int c, Box bbox) {
		while (c-- > 0) {
	...
	private void fit(Position[] a, int cx, Vertex[] ix, int fudge, Box B) {
		int c = cx;
		while (c-- > 0) {
			ix[c] = new Vertex();
			ix[c].ip = new IPoint();
			ix[c].ip.x = ((int) ((a[c].X - B.min.x) * sclx - mid) & ~7) | fudge | (c & 1);
			ix[c].ip.y = ((int) ((a[c].Y - B.min.y) * scly - mid) & ~7) | fudge;
		}
		...
		c = cx;
		while (c-- > 0) {
			ix[c].rx = ix[c].ip.x < ix[c + 1].ip.x ? new Rng(ix[c].ip.x, ix[c + 1].ip.x)
					: new Rng(ix[c + 1].ip.x, ix[c].ip.x);
			ix[c].ry = ix[c].ip.y < ix[c + 1].ip.y ? new Rng(ix[c].ip.y, ix[c + 1].ip.y)
	...
	private double inter(Position[] a, Position[] b) {
		int na = a.length;
		int nb = b.length;
	...
	private static Position[] toPositionArray(double[][] a) {
		Position[] A = new Position[a.length];
		for (int i = 0; i < a.length; i++)
			A[i] = new Position(a[i][0], a[i][1]);
		return A;
	}

	private static void trial(double[][] a, double[][] b) {
		Position[] A = toPositionArray(a);
		Position[] B = toPositionArray(b);
	...
	public static void main(String[] args) {
		...
		double b1[][] = { { 1, 1 }, { 1, 4 }, { 4, 4 }, { 4, 1 }, { 1, 1 } }; // 1/2,
																				// 1/2
		...
		double b2[][] = { { 3, 1 }, { 5, 1 }, { 5, 4 }, { 3, 4 }, { 3, 5 }, { 6, 5 }, { 6, 0 }, { 3, 0 } }; // 0,
																											// 9
		...
		double a9[][] = { { 0, 0 }, { 0, 2 }, { 2, 2 }, { 2, 0 } };
		double b9[][] = { { 1, 1 }, { 1, 3 }, { 3, 3 }, { 3, 2 } }; // 
		trial(a9, b9);
		
		double a11[][] = { { 0, 0 }, { 0, 2 }, { 2, 2 }, { 2, 0 } };
		double b11[][] = { { 3, 3 }, { 3, 5 }, { 5, 5 }, { 5, 3 } }; // 
		trial(a11, b11);
		
		double a12[][] = { { 0, 0 }, { 0, 2 }, { 2, 2 }, { 2, 0 } };
		double b12[][] = { { 0, 0 }, { 3, 5 }, { 5, 5 }, { 5, 3 } }; // 
		trial(a12, b12);
	}
}
